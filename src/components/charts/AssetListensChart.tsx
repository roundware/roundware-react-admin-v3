...
const AssetListensChart = ({ assets, listenEvents }: Props): JSX.Element => {
  ...
  useEffect(() => {
    if (assets?.data.length) {
      setAssetViewIndex([0, assets.data.length]);
    }
  }, [assets]);
  ...
  const perAssetListens: BarChartData[] = useMemo(() => {
    ...
    return assets.data
      .map((asset) => ({
        assetId: asset.id,
        total: listenEvents.data.filter((listen) => listen.asset_id == asset.id)
          .length,
      }))
      .sort((a) => a.assetId)
          ...
          <div style={{ width: "100%", height: 300 }}>
            <ResponsiveContainer>
              <ComposedChart data={perAssetListens}>
                ...
                <YAxis dataKey={"total"} type="number" label={"Listens"} />
